...
from mmdet.models.utils.builder import build_transformer
...
def pos2posemb3d(pos, num_pos_feats=128, temperature=10000):
    """Sinusoidal embedding of normalized 3D positions used for queries."""
    scale = 2 * math.pi
    pos = pos * scale
...
class PETRHead(AnchorFreeHead):
    """3D detection head for PETR.

    Args:
        num_classes (int): Number of foreground classes.
        in_channels (int): Number of channels in the input feature map.
        num_query (int): Number of object queries.
        num_reg_fcs (int): Number of FFN layers in cls/reg branches.
        transformer (dict): Config for the transformer.
        sync_cls_avg_factor (bool): Whether to sync the avg_factor across GPUs.
        positional_encoding (dict): Config for the positional encoding.
        code_weights (list[float]): Per-channel weight for the regression loss.
        bbox_coder (dict): Config for the box coder used to decode predictions.
        loss_cls (dict): Config of the classification loss.
        loss_bbox (dict): Config of the regression loss.
        loss_iou (dict): Config of the optional IoU loss (unused by default).
        train_cfg (dict): Training config containing the matcher.
        test_cfg (dict): Testing config (max_per_img and similar).
        with_position (bool): Whether to inject the projected 3D coordinate
            embedding into the encoder.
        with_multiview (bool): Whether the sine encoding spans all views.
        depth_step (float): Stride between depth bins (legacy, unused if LID).
        depth_num (int): Number of depth bins.
        LID (bool): Use Linear-Increasing-Discretization for the depth grid.
        depth_start (float): Closest depth used for the depth grid.
        position_range (list[float]): Range of valid 3D positions.
        normedlinear (bool): Use a normalized linear layer for classification.
    """
    _version = 2
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is PETRHead):
            assert isinstance(class_weight, float), (
                'Expected class_weight to have type float. Found '
                f'{type(class_weight)}.')
            bg_cls_weight = loss_cls.get('bg_cls_weight', class_weight)
            assert isinstance(bg_cls_weight, float), (
                'Expected bg_cls_weight to have type float. Found '
        ...
        # Defer the real layer construction until after we have built the
        # positional encoding, transformer and bbox coder. AnchorFreeHead's
        # __init__ unconditionally calls ``_init_layers``; the guard makes
        # that first call a no-op.
        self._petr_layers_built = False
        ...
        self._petr_layers_built = True
        self._init_layers()

    def _init_layers(self):
        """Build the projection, query, regression and classification heads."""
        if not getattr(self, '_petr_layers_built', True):
            return
    ...
    def position_embeding(self, img_feats, img_metas, masks=None):
        """Compute the 3D position embedding for the encoder feature map."""
        eps = 1e-5
        pad_shape = img_metas[0]['pad_shape']
        if isinstance(pad_shape, (list, tuple)) and len(pad_shape) > 0 and \
                isinstance(pad_shape[0], (list, tuple)):
            pad_h, pad_w = pad_shape[0][:2]
        else:
        ...
        D = coords_d.shape[0]
        coords = torch.stack(
            torch.meshgrid([coords_w, coords_h, coords_d])).permute(
                1, 2, 3, 0)  # W, H, D, 3
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        """Map state dicts that pre-date the AnchorFreeHead refactor."""
    ...
    def forward(self, mlvl_feats, img_metas):
        """Run the PETR head over the multi-view feature pyramid.

        Args:
            mlvl_feats (tuple[Tensor]): Encoder features, each shape
                ``[B, N, C, H, W]``.
            img_metas (list[dict]): Per-sample metadata containing
                ``pad_shape``, ``img_shape`` and ``lidar2img``.

        Returns:
            dict: Stacked predictions with keys
            ``all_cls_scores``, ``all_bbox_preds``,
            ``enc_cls_scores`` (None) and ``enc_bbox_preds`` (None).
        """
        x = mlvl_feats[0]
        batch_size, num_cams = x.size(0), x.size(1)

        # Padded shape lookup. Different upstream pipelines store this either
        # as a single (H, W, C) tuple or as a per-camera list of tuples.
        pad_shape = img_metas[0]['pad_shape']
        if isinstance(pad_shape, (list, tuple)) and len(pad_shape) > 0 and \
                isinstance(pad_shape[0], (list, tuple)):
            input_img_h, input_img_w = pad_shape[0][:2]
        else:
            input_img_h, input_img_w = pad_shape[:2]
        ...
        for img_id in range(batch_size):
            ...
            for cam_id in range(num_cams):
                if isinstance(img_shape, (list, tuple)) and len(img_shape) > 0 \
                    ...
                    img_h, img_w = img_shape[cam_id][:2]
                else:
    ...
    def _get_target_single(self,
        ...
        """Compute the targets for a single image."""
    ...
    def get_targets(self,
        ...
        """Aggregate :func:`_get_target_single` over a batch."""
    ...
    def loss_single(self,
                    ...
                    gt_labels_list,
                    gt_bboxes_ignore_list=None):
        """Loss computation for a single decoder layer."""
    ...
    def loss(self,
        ...
        if enc_cls_scores is not None:
            ...
            enc_loss_cls, enc_losses_bbox = self.loss_single(
                enc_cls_scores, enc_bbox_preds, gt_bboxes_list,
                binary_labels_list, gt_bboxes_ignore)
    ...
    def get_bboxes(self, preds_dicts, img_metas, rescale=False):
        """Decode the predictions and wrap them as 3D bbox containers."""
