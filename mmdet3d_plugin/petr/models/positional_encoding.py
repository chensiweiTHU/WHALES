...
class SinePositionalEncoding3D(BaseModule):
    """Sine/cosine positional encoding for multi-view feature maps.

    Produces a tensor of shape ``[bs, num_views, 3*num_feats, h, w]`` so that
    PETRHead can fuse it with the projected pixel-frustum encoding.
    """
    ...
    def __init__(self,
        ...
        if normalize:
            assert isinstance(scale, (float, int)), (
                'when normalize is set, scale should be float/int, found '
                f'{type(scale)}')
...
class LearnedPositionalEncoding3D(BaseModule):
    """Learnable positional encoding factored into row/column tables."""
