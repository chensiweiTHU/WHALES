...
from ...core.bbox import (CameraInstance3DBoxes, DepthInstance3DBoxes,
...
class RandomFlip3D(RandomFlip):
    ...
    def random_flip_data_3d(self, input_dict, direction='horizontal'):
        ...
        if 'lidar2img' in input_dict:
            if self.sync_2d and direction == 'horizontal':
                # Get image width
                if 'img' in input_dict:
                    if isinstance(input_dict['img'], list):
                        w = input_dict['img'][0].shape[1]
                    else:
                        w = input_dict['img'].shape[1]
                elif 'img_shape' in input_dict:
                    w = input_dict['img_shape'][1]
                elif 'ori_shape' in input_dict:
                    w = input_dict['ori_shape'][1]
                else:
                    raise ValueError("Cannot determine image width for lidar2img flip")
                
                # Create horizontal flip matrix for image coordinates
                # Flip x: x_new = w - x_old
                ...
                # Apply flip to all lidar2img matrices
                flipped_lidar2img = []
                for lidar2img in input_dict['lidar2img']:
                    lidar2img = np.array(lidar2img)
                    if lidar2img.shape == (4, 4):
                        # Extract 3x4 projection part
                        projection = lidar2img[:3, :]
                    else:
                        projection = lidar2img
                    
                    # Apply flip: flip_matrix @ projection
                    flipped_projection = flip_matrix @ projection
                    
                    # Reconstruct 4x4 if needed
                    if lidar2img.shape == (4, 4):
                        flipped_lidar2img_mat = np.eye(4, dtype=np.float32)
                        flipped_lidar2img_mat[:3, :] = flipped_projection
                    else:
                        flipped_lidar2img_mat = flipped_projection
                    
                    flipped_lidar2img.append(flipped_lidar2img_mat)
                
                input_dict['lidar2img'] = flipped_lidar2img
