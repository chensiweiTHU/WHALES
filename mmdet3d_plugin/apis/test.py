...
def single_gpu_test(model,
    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            ...
            if hasattr(dataset, 'history_results'):
                dataset.history_results[i] = result
            if 'ego' in data.keys(): # opencood dataset
                del data['ego'][0]['origin_lidar']
                del data['ego'][0]['processed_lidar']
                del data['ego'][0]['anchor_box']
                result[0]['data_dict'] = dict()
                result[0]['data_dict']['ego'] = data['ego'][0]
    ...
    if object_fusion:
        print('start object fusion........')
        if hasattr(model.module, 'pts_bbox_head'):
            head = model.module.pts_bbox_head
        elif hasattr(model.module, 'dense_head'):
            head = model.module.dense_head
        test_cfg = head.test_cfg
        results = get_object_fusion_results(results,dataset,test_cfg,model.src_device_obj)
    return results
...
def get_object_fusion_results(results,dataset,test_cfg,device):
    ...
    for i, oldresult in enumerate(results):
        ...
        veh_num = dataset.data_infos[i]['sample_info']['vehicle_num']
        assert len(result['img_metas'])==1
        if 'cooperative_agents' in result['img_metas'][0]:
            print(result['img_metas'][0].keys())    
            ego_token = result['img_metas'][0]['ego_agent']['sample_idx']
            ...
            if fuse_all:
                ...
                for j in range(veh_num):
                    # veh_token = dataset.fuse_token(ego_scene,ego_frame,j)
                    if j!=ego_veh:
                        key_list.append(str(j))
            else:
                key_list = list(veh_token in result['img_metas'][0]['cooperative_agents'].keys())
            for veh_token in key_list:
...
def merge_results(recovered_bboxes, recovered_scores, recovered_labels, test_cfg, device):
    ...
    if "use_rotate_nms" in test_cfg.keys() and  test_cfg.use_rotate_nms:
        nms_func = nms_gpu
    else:
    ...
    if 'max_num' in test_cfg:
        max_num = test_cfg.max_num
    else:
        max_num = 300
    num = min(max_num, len(aug_bboxes))
    order = order[:num]
