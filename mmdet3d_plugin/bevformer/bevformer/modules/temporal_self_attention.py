...
class TemporalSelfAttention(BaseModule):
    ...
    When ``prev_bev`` is None (single-frame mode), the previous BEV is set to
    a copy of the current query, so this layer behaves like a vanilla
    deformable self-attention. ``num_bev_queue`` is fixed to 2 to match
    upstream BEVFormer.
    """
    ...
    def forward(self,
                ...
                flag='decoder',
                **kwargs):
        """Temporal self-attention forward."""
        if value is None:
