...
class VoxelNeXtCoopertivePruningImportance(VoxelNeXtCoopertive):
    ...
    def __init__(self, pts_voxel_layer,pts_voxel_encoder,
                  ...
                  #num_class, dataset):
        super(VoxelNeXtCoopertivePruningImportance,self).__init__(pts_voxel_layer,pts_voxel_encoder,
    ...
    def forward_train(self,
        ...
        if not self.single and self.proj_first:
            ...
            print('wrong! projection has been done in the pipeline')
        ...
        if not self.single and not self.raw:
            ...
            important_coords,important_voxels, voxel_feats_inf = self.pruning(voxel_feats_inf)
        ...
        if not self.single and self.raw:
            for i in range(len(infrastructure_points)):
                points[i] = torch.cat([points[i],infrastructure_points[i]],dim=0)
        elif not self.single and not self.raw:
            for i in range(len(compressed_points_inf)):
                points[i] = torch.cat([points[i],compressed_points_inf[i]],dim=0)
            # vis = True
            # if vis:
                
        img_feats, voxel_feats = self.extract_feat(
            points, img=img, img_metas=img_metas)
        # print('voxel_feats',voxel_feats['voxels'].shape)
        voxel_feats.update(gt_boxes=bboxes_3d_tensor)
        pts_feats = self.backbone_3d(voxel_feats)
        # print('pts_feats',pts_feats['encoded_spconv_tensor'].features.shape)
