...
class VoxelNeXtCoopertiveMultiscale(Base3DDetector):
    ...
    def forward_train(self,
        ...
        if history_data is not None and len(history_data) > 0:
            for i in range(len(history_data)-1,-1,-1):
                ...
                #     box = boxes[i]
                #     mask = (points[0][:,0] > box[0]) &\
                #         (points[0][:,0] < box[3]) &\
                #         (points[0][:,1] > box[1]) &\
                save = False
                if save:
                    heat_map = heat_map.cpu().numpy()
                    np.save('heat_map.npy',heat_map[0,0,:,:])
    ...
    def simple_test(self,points=None,img_metas=None,img=None,infrastructure_points=None,infrastructure_img=None,history_data=None,**kwargs):
        "we cannot project points in test pipelines, so we need to project them here"
        device = points[0].device
        for i,points_inf in enumerate(infrastructure_points[0]):
            if len(points_inf) == 0:
                points_inf = torch.zeros((1,4)).to(device)
                infrastructure_points[0][i] = points_inf
        img_feats, voxel_feats = self.extract_feat(
            points, img=img, img_metas=img_metas)
        pts_feats = self.backbone_3d(voxel_feats)
        if history_data is not None and len(history_data) > 0:
            for i in range(len(history_data)-1,-1,-1):
                ...
                if i == 0:
                    ...
                    return_BEV = True
                his_pts_feats = self.get_history_results(history_data[i]['points'][0],\
                    history_data[i]['infrastructure_points'][0],img_metas,history_data[i]['img_metas'][0],\
                    older_BEV,return_BEV)
                # his_pts_feats = self.temporal_fusion(pts_feats, his_output_dict, img_metas)
        else:
            his_pts_feats = None
        if not self.single and not self.raw:
            img_feats_inf, voxel_feats_inf = self.extract_feat(
            infrastructure_points[0], img=infrastructure_img, img_metas=img_metas)
            pts_feats_inf = self.inf_backbone_3d(voxel_feats_inf)
            pts_feats = self.feature_fusion(pts_feats, pts_feats_inf, img_metas)
        if his_pts_feats is not None:
            pts_feats = self.temporal_fusion(pts_feats, his_pts_feats, img_metas)
