...
class MSDeformAttn(nn.Module):
    ...
    def forward(self, query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask=None):
        ...
        print('Len_in_shape:', input_flatten.shape)
        print('input_spatial_shapes:', (input_spatial_shapes[:, 0] * input_spatial_shapes[:, 1]).sum())
        ...
        if reference_points.shape[-1] == 2:
            offset_normalizer = torch.stack(
                [input_spatial_shapes[..., 1], input_spatial_shapes[..., 0]], -1).to(sampling_offsets)  
...
class Deform_Transformer(nn.Module):
    ...
    def forward(
        ...
        reference_points = center_pos[:, :, None, :].repeat(
            1, 1, self.levels, 1)  
...
class RPN_transformer_deformable_mtf_singlescale(nn.Module):
    ...
    def forward(self, x, psm):
        ...
        scores, labels = torch.max(psm_sum.reshape(
            1, 1, H * W), dim=1)  
        ...
        order = sort[1] 
        order = order[:, :obj_num]
        batch_id = torch.from_numpy(np.indices((1, obj_num))[
                                    0]).to(labels)  
        ...
        temp = x_ego.reshape(1, -1, H * W).transpose(2,
                                                     1).contiguous()
        ct_feat = (
...
if __name__ == '__main__':
    model = RPN_transformer_deformable_mtf_singlescale(
            channel=256, points=9).to('cuda')
    print(model)
    print(dir(model))
    print(torch.cuda.is_available())
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ms = model(torch.randn(5, 256, 64, 64).to(device), [torch.randn(1, 64, 64).to(device) for _ in range(5)])
