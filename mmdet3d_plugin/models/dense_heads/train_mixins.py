...
class AnchorTrainMixin(object):
    ...
    def anchor_target_single_assigner(self,
        ...
        if len(gt_bboxes) > 0:
            ...
            if gt_bboxes.size(-1) == 7 and anchors.size(-1) == 9:
                zeros = gt_bboxes.new_zeros(gt_bboxes.shape[:-1] + (2,))
                gt_bboxes = torch.cat([gt_bboxes, zeros], dim=-1)
                # print("Warning: gt_bboxes has 7 dims, but anchors have 9 dims.")
