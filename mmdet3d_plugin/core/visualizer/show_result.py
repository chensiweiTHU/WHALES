...
def _write_box(scene,out_filename):
...
def _write_oriented_bbox(scene_bbox, out_filename):
    ...
    return
def _write_bbox2scene(scene, scene_bbox,color):
    """Export oriented (around Z axis) scene bbox to meshes.
    ...
    def convert_oriented_box_to_trimesh_fmt(box):
        ...
        trns[0:3, 0:3] = heading2rotmat(box[6])
        box_trimesh_fmt = trimesh.creation.box(lengths, trns)
        box_trimesh_fmt.visual.face_colors = [color for i in range(12)]
        box_trimesh_fmt.visual.vertex_colors = [color for i in range(8)]
        return box_trimesh_fmt
...
def _write_all(scene_bbox,points, out_filename):
    """Export oriented (around Z axis) scene bbox to meshes.
    ...
    def create_tiny_box_point(point):
        trns = np.eye(4)
        lengths = [0.05, 0.05, 0.05]
        trns[0:3, 3] = point[:3]
        box_trimesh_fmt = trimesh.creation.box(lengths, trns)
        return box_trimesh_fmt
    ...
    if len(scene_bbox) == 0:
        scene_bbox = np.zeros((1, 7))
    mesh = trimesh.Trimesh(vertices=points[:,:3],)
    scene = trimesh.scene.Scene()
    ...
    for i in range(points.shape[0]):
        b = create_tiny_box_point(points[i])
...
def show_result(points,
    ...
    if gt_bboxes is not None:
        ...
        _write_all(gt_bboxes,points, osp.join(result_path, f'{filename}_gt_all.obj'))
    if pred_bboxes is not None:
        # bottom center to gravity center
        pred_bboxes[..., 2] += pred_bboxes[..., 5] / 2
        # the positive direction for yaw in meshlab is clockwise
        pred_bboxes[:, 6] *= -1
        print("pred_bboxes",pred_bboxes.shape[0])

        "put gt and pred together"
        # all_bboxes = np.concatenate([gt_bboxes, pred_bboxes], axis=0)
        all_bboxes = pred_bboxes
        # scene = trimesh.scene.Scene()
        # scene = _write_bbox2scene(scene,gt_bboxes,[0,255,0,255])
        # scene = _write_bbox2scene(scene,pred_bboxes,[255,0,0,255])
        # _write_box(scene, osp.join(result_path, f'{filename}_pred_gt.obj'))

        # _write_oriented_bbox(all_bboxes,
        #                      osp.join(result_path, f'{filename}_pred_gt.obj'))
        _write_all(all_bboxes,points, osp.join(result_path, f'{filename}_pred_gt_all.obj'))
