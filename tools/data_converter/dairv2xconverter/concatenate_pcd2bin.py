...
def concatenate_pcd2bin(path1, path2, path_save):
    try:
        ...
        np_x1 = np.array([], dtype=np.float32)
        np_y1 = np.array([], dtype=np.float32)
        np_z1 = np.array([], dtype=np.float32)
        np_i1 = np.array([], dtype=np.float32)
        print("infra_points not found")
