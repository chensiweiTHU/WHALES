...
class Dolphins:
    ...
    def __init__(self,
        ...
        self.table_names = ['sample']
        self.table_root = self.dataroot
        self.label_map = {"Vehicle":0, "Pedestrian":1, "Cyclist":2}
        self.natural_sort_key = lambda x: [int(text) if text.isdigit() else text.lower() for text in re.split('([0-9]+)', x)]
        
        self.scenes = [scene for scene in sorted(os.listdir(self.table_root), key=self.natural_sort_key)
                if os.path.isdir(os.path.join(self.table_root, scene)) and self.isScene(scene)]
        if train_test == 'prepare':
            pass
            ...
            raise ValueError('train_test should be either prepare, train or test')
        self.scene_name = "scenen"
        self.config_name = "config"
        self.frame_name = "frame_info"
        self.version = version
        # assert osp.exists(self.table_root), 'Database version not found: {}'.format(self.table_root)

        start_time = time.time()
        if verbose:
            print("================================================================\nLoading scenes, frames and configs for dolphins...")

        # Explicitly assign tables to help the IDE determine valid class members.
        
        self.scenen = self.__load_scene__(self.scene_name)
        self.config = self.__load_config__(self.config_name)
        self.frames = self.__load_table__(self.frame_name)
        self.sample = self.get_sample()
        # for dir in self.dirs:
        #     if not dir.startswith('scene') and not dir.startswith('config'):
        #         frame_info_path = str(dir) + '/frame_info'
        #         self.frames.append(self.__load_table__(frame_info_path))
        zero_v=[]
        for sample_id in range(len(self.sample)):
            for ann_id in range(len(self.sample[sample_id]['sample_annotation'])):
                size = self.sample[sample_id]['sample_annotation'][ann_id]['size']
                volume = size[0]*size[1]*size[2]
                if volume <0.01:
                    print('volume=0')
                    zero_v.append([sample_id, ann_id])
        if verbose:
    ...
    def get_statistics(self):
        ...
        print(f'num of valid samples: {num_samples}')
        print('total number of images: ', 4 * len(self.sample))
        print('total number of  point clouds:',  len(self.sample) )
        print('counting annotations...')
        num_view_pts=0
        min_view_pts=np.inf
        max_view_pts=0
        for sample in self.sample:
            num_view_pts += sample['vehicle_num']+1
            min_view_pts = min(min_view_pts, sample['vehicle_num']+1)
            max_view_pts = max(max_view_pts, sample['vehicle_num']+1)
        print('mean number of view points:', num_view_pts/num_samples)
        print('min number of view points:', min_view_pts)
        print('max number of view points:', max_view_pts)
        num_annos = 0
        for sample in self.sample:
            # num_annos += len(sample['sample_annotation'])
            for veh_location in sample['veh_locations']:
                for anno in sample['sample_annotation']:
                    if np.linalg.norm( np.array(anno['location'])-np.array(veh_location))<50:
                        num_annos += 1
        print(f'num of valid annotations: {num_annos}')
        num_annos = 0
        for sample in self.sample:
            # num_annos += len(sample['sample_annotation'])
            for anno in sample['sample_annotation']:
                for veh_location in sample['veh_locations']:
                    ...
                    if np.linalg.norm( np.array(anno['location'])-np.array(veh_location))<50:
                        num_annos += 1
                        break
        print(f'num of unique valid annotations: {num_annos}')
    def get_sample(self):
        samples = []
        for scene in self.scenes:
            steps = len(self.frames[scene])
            interval = self.config[scene]['world']['save_interval'] 
            time_interval = interval*0.1
            vehicle_num = self.scenen[scene]['vehicle_num']
            for step in range(1,steps+1):
                for v_id in range(vehicle_num+1):
                    sample_token = f'{scene}_{step*interval}_{v_id}'
                    if step>1:
                        prev_torken = f'{scene}_{(step-1)*interval}_{v_id}'
                    else:
                        prev_torken = ''
                    if step<steps:
                        next_torken = f'{scene}_{(step+1)*interval}_{v_id}'
                    else:
                        next_torken = ''
                    sample = {
                        "token": sample_token,
                        "scene_token": scene,
                        "prev_token": prev_torken,
                        "next_token": next_torken,
                        "timestamp": step*time_interval,
                        "vehicle_index": v_id,
                    }

                    sample_data = self.frames[scene][f'{scene}_{step*interval}']
                    sample_data['veh_id'] = v_id
                    sample.update(sample_data)
                    samples.append(sample)     
        return samples
        # samples = []
        # for scene_token in self.frames.keys():
        #     for frame_token in self.frames[scene_token].keys():
        #         token = frame_token
        #         self.frames[scene_token][frame_token]['sample'] = self.frames[scene_token][frame_token]['sample_annotation']
    
    def __load_scene__(self, scene_name) -> dict:
        """ Loads scenes under self.table_root. """
        scenes = {}
        for scene_token in sorted(os.listdir(self.table_root), key=self.natural_sort_key):
            if scene_token in self.scenes:
                scene_path = osp.join(self.table_root, scene_token)
                if osp.isdir(scene_path) and self.isScene(scene_token):
                    with open(osp.join(scene_path, f'{scene_name}.json'), 'r') as f:
                        scene_data = json.load(f)

                    scenes[scene_token] = scene_data
    ...
    def __load_table__(self, table_name) -> dict:
        """ Loads tables under self.table_root. """
        table = {}
        ...
        for scene_token in sorted(os.listdir(self.table_root), key=self.natural_sort_key):
            if scene_token in self.scenes:
                scene_path = osp.join(self.table_root, scene_token)
                if osp.isdir(scene_path) and self.isScene(scene_token):
                    with open(osp.join(scene_path, 'scenen.json'), 'r') as f:
                        scene_data = json.load(f)

                    type_to_token = {category["name"]: category["token"] for category in scene_data["categories"]}

                    table[scene_token] = {}

                    frame_dirs = sorted(os.listdir(scene_path), key=self.natural_sort_key)
                    for frame_dir in frame_dirs:
                        frame_dir_path = osp.join(scene_path, frame_dir)
                        if osp.isdir(frame_dir_path):
                            frame_info_path = osp.join(frame_dir_path, f'{table_name}.json')
                            if osp.exists(frame_info_path):
                                with open(frame_info_path, 'r') as f:
                                    frame_data = json.load(f)

                                sample_annotations = frame_data.get('sample_annotation', [])
                                for obj in sample_annotations:
                                    obj_type_token = type_to_token.get(obj["type"])
                                    obj_token = "{}_{}_{}_{}".format(scene_token, frame_dir, obj['index'], obj_type_token)
                                    obj['token'] = obj_token
                                for ann_id in range(len(sample_annotations)):
                                    for i in range(3):
                                        if sample_annotations[ann_id]['size'][i]<0.15:
                                            sample_annotations[ann_id]['size'][i] = 0.15
                                frame_data['sample_annotation'] = sample_annotations
                                frame_token = "{}_{}".format(scene_token, frame_dir)
                                table[scene_token][frame_token] = frame_data
    ...
    def __load_config__(self, config_name) -> dict:
        """ Loads configs under self.table_root. """
        config = {}
        ...
        for scene_token in sorted(os.listdir(self.table_root), key=self.natural_sort_key):
            if scene_token in self.scenes:
                scene_path = osp.join(self.table_root, scene_token)
                if osp.isdir(scene_path) and self.isScene(scene_token):
                    with open(osp.join(scene_path, f'{config_name}.yaml')) as f:
                        scene_config = yaml.load(f, Loader=yaml.Loader)
                        del scene_config["description"]
                        scene_config["scene_token"] = scene_token
                        config[scene_token] = scene_config
    ...
    def isScene(self, dir_name) -> bool:
        try:
    ...
    def get_sample_data(self, step: int, scene_token: str, vehicle_index: int, save_interval: int,
        ...
        annotations = self.frames[scene_token][f'{scene_token}_{step*save_interval}']['sample_annotation']
        vehicle_locations = self.frames[scene_token][f'{scene_token}_{step*save_interval}']['veh_locations']
        vehicle_rotations = self.frames[scene_token][f'{scene_token}_{step*save_interval}']['veh_rotations']

        boxes = self.get_boxes(step, save_interval, scene_token)
        ...
        for box in boxes:
            ...
            if use_flat_vehicle_coordinates:
                ...
                # hard code the height of the vehicle
                if vehicle_index<len(vehicle_locations)-1:
                    translation = np.array([0, 0, 1.8])+np.array(vehicle_locations[vehicle_index])
                else: # the rsu is the sensor
                    translation = np.array(vehicle_locations[vehicle_index])
        ...
        return box_list, cam_intrinsic, annotations, vehicle_locations, vehicle_rotations

    def get_box(self, curr_anno) -> Box:
        """
        ...
        "turn over x axis"
        # curr_anno['rotation'][1] = -curr_anno['rotation'][1]
        # curr_anno['location'][0] = -curr_anno['location'][0]
        return Box(curr_anno['location'], size, Quaternion(curr_anno['rotation']),
                   name=curr_anno['type'], token=curr_anno['token'], label=label)

    def get_boxes(self, step: int, interval: int, index: str) -> List[Box]:
        """
        ...
        # label_map = dict()
        # for cat in scene_info['categories']:
        #     label_map[cat['name']] = cat['token']
        boxes = list(self.get_box(curr_anno) for curr_anno in self.frames[index][f'{index}_{step*interval}']['sample_annotation'])
        return boxes
    
    def box_velocity(self, scene_token: str, save_interval: int, step: int, anno, time_interval: float, max_time_diff: float = 1.5) -> np.ndarray:
        """
        ...
        id = anno['index']
        # current = self.frames[scene_token][f'{scene_token}_{(step)*save_interval}']['sample_annotation']

        current = [a for a in self.frames[scene_token][f'{scene_token}_{step*save_interval}']['sample_annotation'] if a['index'] == id][0]

        # prev_annotations = self.frames[scene_token][f'{scene_token}_{(step-1)*save_interval}']['sample_annotation']
        # next_annotations = self.frames[scene_token][f'{scene_token}_{(step+1)*save_interval}']['sample_annotation']
        prev_annotations = self.frames[scene_token].get(f'{scene_token}_{(step-1)*save_interval}', {}).get('sample_annotation', [])
        next_annotations = self.frames[scene_token].get(f'{scene_token}_{(step+1)*save_interval}', {}).get('sample_annotation', [])

        has_prev, has_next = False, False
        first, last = None, None
        
        for prev_anno in prev_annotations:
            if prev_anno['index'] == id:
                has_prev = True
                first = prev_anno
                break
        for next_anno in next_annotations:
            if next_anno['index'] == id:
                has_next = True
                last = next_anno
                break
        ...
        if not has_prev:
            # Use forward difference if only next frame is available.
            first = current
            time_diff = time_interval
        elif not has_next:
            # Use backward difference if only previous frame is available.
            last = current
            time_diff = time_interval
        else:
            # Use centered difference if both frames are available.
            time_diff = 2 * time_interval

        pos_last = np.array(last['location'])
        pos_first = np.array(first['location'])
        ...
        if time_diff > max_time_diff:
            # If time_diff is too significant, don't return an estimate.
            return np.array([np.nan, np.nan, np.nan])
        else:
            # print(pos_diff / time_diff)
...
class NuScenesExplorer:
    ...
    def __init__(self, nusc: Dolphins):
        self.nusc = nusc
...
if __name__ == "__main__":
    dolphins = Dolphins('./data/Dolphins')