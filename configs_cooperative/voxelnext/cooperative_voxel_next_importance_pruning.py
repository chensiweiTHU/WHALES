...
model = dict(
    type='VoxelNeXtCoopertivePruningImportance',
    ...
    pruning=dict(
        type='VoxelResSPSAttnQuantiseizer',
        input_channels = num_point_features,
        grid_size = grid_size,
        spconv_kernel_sizes=[3,3,3,3], 
        channels=[32,64,128,256], 
        point_cloud_range=[-3, -46.08, 0, 1, 46.08, 92.16],
        downsample_pruning_ratio = [0.5,0.5,0.5,0.8], #first 3 are for downsample, last one is for total pruning
    ),
    backbone_3d=dict(
...
log_level = 'INFO'
load_from = "work_dirs/cooperative_voxel_next_raw_level/epoch_3_best.pth"
resume_from = None
workflow = [('train', 1)]
